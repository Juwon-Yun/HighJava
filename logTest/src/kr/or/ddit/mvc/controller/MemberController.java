...
public class MemberController {
	...
	public void startMember() {			
		...
		private void excuteExel() {
			System.out.println();
			sc.nextLine();
			System.out.println("출력할 해당 회원정보를 입력하세요");
			System.out.print("출력할 회원 ID >>");
			String memId = sc.nextLine();
			
			int count = service.getMemberCount(memId);
			if( count ==0 ) {
				System.out.println(memId + "은(는) 없는 회원ID 입니다");
				System.out.println("엑셀 출력 작업을 종료합니다.");
				return;
			}
			...
			do {
				System.out.println();
				System.out.println("엑셀로 출력할 항목을 선택하세요");
				System.out.println("1.비밀번호  2.회원이름  3.전화번호  4.회원주소");
				System.out.println("--------------------------------------------------------");
				System.out.print("출력 항목 선택 >> ");
				num = sc.nextInt();
				...
				case 4: updateField = "mem_addr";
					...
					break;
				default:
					System.out.println("출력할 항목을 잘못 선택했습니다");
					System.out.println("다시 선택하세요");
				}
			} while (num < 1 || num > 4);
			System.out.println();
			...
			sc.nextLine();
			// 수정할 값 입력받기
			System.out.println("출력할 " + updateTitle + ">>");
			String updateData = sc.nextLine();
			...
			int cnt = service.excuteExel(paramMap);
			
			if(cnt > 0 ) {
				System.out.println("출력 작업 성공");
			}else {
	...
	private int displayMenu() {
		...
		System.out.println("	6. Excel로 출력하기  ");
