...
public class CryptoUtil {
	/**
	 * 매개변수의 문자열을 SHA-512방식으로 해시(암호화)한다
	 * 	(암호화 알고리즘 분류 : MD5, SHA256, SHA3, SHA512, ...)
	 * 
	 * @param str 암호화할 문자열
	 * @return 암호화된 문자열
	...
	public static String encryptAES256( String str, String key) throws UnsupportedEncodingException, NoSuchAlgorithmException, NoSuchPaddingException, InvalidKeyException, InvalidAlgorithmParameterException, IllegalBlockSizeException, BadPaddingException{
		...
		// 암호화할때 128비트씩 끈어서 암호화 하기 때문에 16바이트가 쓰여진다
		byte [] keyBytes = new byte[16];
		
		// arraycopy => 배열복사 					keyBytes배열을 0번째부터 keyBytes의 길이(갯수)만큼 복사한다
		System.arraycopy(key.getBytes("UTF-8"), 0, keyBytes, 0, keyBytes.length);
	
		// 비밀키 생성
		SecretKeySpec keySpec = new SecretKeySpec(keyBytes, "AES");
		
		// Cihper 생성 및 초기화  =>   알고리즘/모드/패딩	(암호화 하는 방식 설정)
		...
		// 패딩 ( Padding ) => 마지막 블록이 블록의길이와 항상 딱 맞아 떨어지지 않기 때문에 부족한 길이만큼
		// 								'0'으로 채우거나 임의의 비트값들로 채워 넣는 방식을 말한다.
